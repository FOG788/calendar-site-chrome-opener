...
予定終了時刻になると、その予定で開いたタブを自動で閉じます。

予定ごとに開き先ウィンドウを変えたい場合は、タイトル/説明/場所に `"[WIN:work]"` または `"#win:work"` を入れてください。
この指定がある予定は、その名前のウィンドウに開きます（なければ自動作成）。
...
- Chromeウィンドウ名: 同じ名前なら同じウィンドウに開く（初回は自動作成）
  - 予定ごとの上書き: 予定テキストに `[WIN:任意名]` または `#win:任意名`
- 何時間先まで予定を取得するか
- 再同期間隔
- 予定時刻を過ぎても開く猶予
- 新しいタブをアクティブにするか
